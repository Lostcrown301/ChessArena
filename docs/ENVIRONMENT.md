...
| Variable | Required | Default | Description |
| :--- | :---: | :--- | :--- |
| `CORS_ORIGIN` | Yes* | `http://localhost:5173` | A comma-separated list of allowed origins. *Required in production*. Must exactly match the frontend URL without trailing slashes. |
...
| Variable | Required | Default | Description |
| :--- | :---: | :--- | :--- |
| `GEMINI_API_KEY` | No | *(empty)* | Google Gemini API key for post-game coaching. If empty, coaching degrades gracefully to Stockfish-only development mode. |
...
4. (Optional) Replace `GEMINI_API_KEY` and `DATABASE_URL` with your credentials.
5. `npm run dev` to start both the frontend and backend.
